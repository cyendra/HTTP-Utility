...
#pragma comment(lib, "WinHttp.lib")
	...
	***********************************************************************/
	...
	void HttpRequest::SetBodyUtf8(const std::wstring& bodyString)
	{
		int utf8Size = WideCharToMultiByte(CP_UTF8, 0, bodyString.c_str(), (int)bodyString.length(), NULL, 0, NULL, NULL);
		char* utf8 = new char[utf8Size + 1];
		ZeroMemory(utf8, utf8Size + 1);
		WideCharToMultiByte(CP_UTF8, 0, bodyString.c_str(), (int)bodyString.length(), utf8, (int)utf8Size, NULL, NULL);
	...
	***********************************************************************/
	...
	std::wstring HttpResponse::GetBodyUtf8()
	{
	...
	***********************************************************************/
	...
	bool HttpQuery(const HttpRequest& request, HttpResponse& response)
	{
		...
		{
			acceptTypes.push_back(request.acceptTypes[i].c_str());
		}
	...
	std::wstring UrlEncodeQuery(const std::wstring& query)
	{
		int utf8Size = WideCharToMultiByte(CP_UTF8, 0, query.c_str(), (int)query.length(), NULL, 0, NULL, NULL);
		char* utf8 = new char[utf8Size + 1];
		ZeroMemory(utf8, utf8Size + 1);
		WideCharToMultiByte(CP_UTF8, 0, query.c_str(), (int)query.length(), utf8, (int)utf8Size, NULL, NULL);
